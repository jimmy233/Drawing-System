...
{
public:
	Quadrilateral(pixelPoint a, pixelPoint b, pixelPoint c, pixelPoint d)
	{
		p.push_back(a);
		p.push_back(b);
		p.push_back(c);
		p.push_back(d);
		strcpy(type, "Quadrilateral");
	}
...
vector<pixelPoint> Quadrilateral::get_control_point()
{
	return p;
}
...
void Quadrilateral::move(int x, int y)
{
	for (int i = 0; i < p.size(); i++)
		p[i] = { p[i].x + x, p[i].y + y };
}
void Quadrilateral::scale(double Sx, double Sy)
{
...
void Quadrilateral::rotate(double angle)
{
	int temp_x = 0, temp_y = 0;
	int n = p.size();
	for (int i = 0; i < n; i++)
...
void Quadrilateral::cut(int xmin, int xmax, int ymin, int ymax)
{
