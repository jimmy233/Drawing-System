...
{
public:
	Triangle(pixelPoint a, pixelPoint b, pixelPoint c)
	{
		p.push_back(a);
		p.push_back(b);
		p.push_back(c);
...
vector<pixelPoint> Triangle::get_control_point()
{
	return p;
}
void Triangle::draw()
...
void Triangle::cut(int xmin, int xmax, int ymin, int ymax)
{
	...
	for (int i = 0; i < cut_num; i++)
	{
		...
		{
			stop = p[j];
			if (IfInWindowOrnot(sp, ep, stop))//inside 
			{
				if (!flag)
				{
					flag = true;
