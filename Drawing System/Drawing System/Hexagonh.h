...
{
public:
	Hexagon(pixelPoint a, pixelPoint b, pixelPoint c, pixelPoint d, pixelPoint e,pixelPoint f)
	{
		p.push_back(a);
		p.push_back(b);
		p.push_back(c);
...
vector<pixelPoint> Hexagon::get_control_point()
{
	return p;
}
void Hexagon::draw()
{
	...
	PointOfMe temp6 = { p[5].x,p[5].y };
	Line line1(temp6, temp2);
	Line line2(temp2, temp3);
	Line line3(temp3, temp1);
	Line line4(temp1, temp4);
	Line line5(temp4, temp5);
	Line line6(temp5, temp6);
	line1.draw(); line2.draw(); line3.draw(); line4.draw(); line5.draw(); line6.draw();
	...
	set<pixelPoint> set6 = line6.get_point_list();
	point_list.insert(set6.begin(), set6.end());
	glFlush();
}

void Hexagon::move(int x, int y)
{
	for (int i = 0; i < p.size(); i++)
		p[i] = { p[i].x + x, p[i].y + y };
}
void Hexagon::scale(double Sx, double Sy)
{
...
void Hexagon::cut(int xmin, int xmax, int ymin, int ymax)
{
